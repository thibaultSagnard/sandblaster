...
"""
In the function has_node(node), I have add 'self.' at self.graph_dict
line 128
"""
...
class Graph():
    ...
    def has_node(self, node):
        return node in self.graph_dict.keys() # I have add the '.self' which was missing

    def update_node(self, node, next_list):
        self.graph_dict[node] = next_list

    def add_new_next_to_node(self, node, next): # work only if graph_dict[node] is a list :?
    ...
    def get_re_index_for_pos(self, regex_list, pos):
        for idx, item in enumerate(regex_list):
            if item["pos"] == pos: # item is so a dic
                return idx
        for idx, item in enumerate(regex_list):
            if item["pos"]-1 == pos: # so pos is an int
                return idx
        return -1
    ...
    def fill_from_regex_list(self, regex_list):
        ...
        self.node_list = []
        for idx, item in enumerate(regex_list): # item is a dict right ?
            node = Node(name="%s" % (idx)) # idx is a number right ?
            ...
            if 'start_node' in item and item['start_node'] == True:
                assert(self.start_node == None) # after 1 change it will not be None so AssertionError
...
## There is an infinit loop in this function I think...
    def get_character_nodes(self, node):
        node_list = []
        for next in self.graph_dict[node]: # /!\ all the nodes in the LIST should be in graph_dict and the next too... perhaps we should verify before if they are nodes...
            if next.is_type_character() or next.is_type_end(): # all the 'next' are also Node objects
                node_list.append(next)
            else:
                node_list = list(set(node_list).union(self.get_character_nodes(next))) #set(L) convertie L en {...}
        return node_list
        ...
        for node in old_dict.keys():
            if node.is_type_jump():
                if self.find_node_type_jump(self.start_node,
                        node, backup_dict):
                    continue
                del self.graph_dict[node] # suppress node in graph_dict
    ...
    def combine_start_end_nodes(self):
        working_strings = self.canon_graph_dict[self.start_state] # table of tuple (string, node)?
        final_strings = []
        string_added = True
        while string_added == True:
            ...
            for (start_string, start_next_state) in initial_strings:
                ...
                if self.canon_graph_dict[start_next_state]:
                    ...
                    final_strings.append((start_string, None)) # why ? in all the cases 'start_next_state in self.end_states' will fail...
                string_added = True
