...
# Add of the object parent in class TerminalNode and NonTerminalNode
        ...
        if self.filter:
            if self.argument:
                        ...
                        if len(s) == 0:
                            ...
                            if "${" in s and "}" in s:
                                if not prefix_added:
                                    ...
                                    curr_filter += "-prefix"
                        if "regex" in curr_filter:
                            ret_str += '(%s #"%s")\n' % (curr_filter, s) # different fron sre_debug
                        else:
                            ret_str += '(%s "%s")\n' % (curr_filter, s)
                    if len(self.argument) == 1:
                        ret_str = ret_str[:-1]
                ...
                if "${" in s and "}" in s:
                    if not "prefix" in curr_filter:
                        curr_filter += "-prefix"
                return "(%s %s)" % (curr_filter, s) # different
            else:
                return "(%s)" % (self.filter) # different
        return "(%02x %04x %04x %04x)" % (self.filter_id, self.argument_id, self.match_offset, self.unmatch_offset)

# str_not is the same as __str__, only 2 return marked as different in the following code in commentary
    def str_not(self):
        if self.filter:
            if self.argument:
                if type(self.argument) is list:
    ...
    def is_last_regular_expression(self): # same as in NonTerminalNode.py, but here num_regex = 0 is defined after
    ...
    def convert_filter(self, convert_fn, f, regex_list, ios_major_version,
            keep_builtin_filters, global_vars, base_addr): # same as in NonterminalNode and we don't know either what is the function convert_fn
...
class OperationNode():
    ...
    def parse_terminal(self, ios_major_version):
        ...
        self.terminal.type = \
            self.raw[2 if ios_major_version <12 else 1] & 0x01 # self.terminal.type will take the value 1 if self.raw[..]=1, 0 else, /!\ it is a binary operation !!!
        self.terminal.flags = \
    ...
    def convert_filter(self, convert_fn, f, regex_list, ios_major_version,
        ...
        if self.is_non_terminal():
            self.non_terminal.convert_filter(convert_fn, f, regex_list,
                ios_major_version, keep_builtin_filters, global_vars, base_addr) # self.non_terminal must be a NonTerminalNode class or an OperationNode one
    ...
    def __hash__(self):
        return struct.unpack('<I', ''.join([chr(v) for v in self.raw[:4]]))[0] # This function converts the strings of binary representations to their original form according to the specified format. The return type of struct.unpack() is always a tuple.
# chr(x) : Get the character that represents the unicode x
...
def ong_mark_not(g, node, parent_node, nodes_to_process): # parent_node & nodes_to_process unused
    g[node]["not"] = True
...
def ong_end_path(g, node, parent_node, nodes_to_process): # same unused variables
    g[node]["decision"] = str(node.non_terminal.match.terminal)
    g[node]["type"].add("final")


def ong_add_to_path(g, node, parent_node, nodes_to_process): # parent_node not used
    if not has_been_processed(node.non_terminal.match):
        g[node]["list"].add(node.non_terminal.match) # function add is for set {} only !
...
def ong_add_to_parent_path(g, node, parent_node, nodes_to_process):
    if not has_been_processed(node.non_terminal.unmatch):
        ...
        nodes_to_process.add((parent_node, node.non_terminal.unmatch)) # /!\ in fact we have had the old node.non_terminal.match because they were inverted

def build_operation_node_graph(node, default_node):
    global processed_nodes # ADD
    if node.is_terminal():
        return None
...
def remove_node_in_operation_node_graph_corr(g, node_to_remove):
    List = deepcopy(g[node_to_remove]["list"]) # ADD
    for n in List: # Modified
        g = remove_edge_in_operation_node_graph(g, node_to_remove, n)
    node_list = list(g.keys())
    for n in node_list:
        if node_to_remove in g[n]["list"]:
...
def _get_operation_node_graph_paths(g, node): # this function modify paths += current_path with current_path=[node, + children if necessary ] (children are in g[node]["list"] )
...
# this function supress nodes in g[node]["list"] which are in start_list and also nodes from their children
...
def clean_edges_in_operation_node_graph(g): # this function can modify a lot g[nodes]["list"]
    ...
    for snode in start_nodes:
        ...
        while True:
            ...
            remove_duplicate_node_edges(g, g[node]["list"]) # ADD g =
...
def clean_nodes_in_operation_node_graph(g):
    ...
    for node_iter in node_list:
        ...
        if g[node_iter]["list"]:
            continue
        logger.warn("going to remove" + str(node_iter)) # print on the terminal; was it the goal ?
        made_change = True
...
class ReducedVertice():
    ...
    def __eq__(self, other): # ADD
        if isinstance(self, ReducedVertice) and isinstance(other, ReducedVertice):
            return self.TYPE_SINGLE==other.TYPE_SINGLE and self.TYPE_START==other.TYPE_START and self.TYPE_REQUIRE_ANY==other.TYPE_REQUIRE_ANY and self.TYPE_REQUIRE_ALL==other.TYPE_REQUIRE_ALL and self.TYPE_REQUIRE_ENTITLEMENT==other.TYPE_REQUIRE_ENTITLEMENT and self.type==other.type and self.is_not==other.is_not and self.value==other.value and self.decision==other.decision
        else:
            return False
    """def _eq_(self, other): # ADD
        if self.TYPE_SINGLE!=other.TYPE_SINGLE:
            #print("type_single diff")
            return False
        if self.TYPE_START!=other.TYPE_START:
            #print("TYPE_START diff")
            return False
        if self.TYPE_REQUIRE_ANY!=other.TYPE_REQUIRE_ANY:
            #print("TYPE_REQUIRE_ANY diff")
            return False
        if self.TYPE_REQUIRE_ALL!=other.TYPE_REQUIRE_ALL:
            #print("TYPE_REQUIRE_ALL diff")
            return False
        if self.TYPE_REQUIRE_ENTITLEMENT!=other.TYPE_REQUIRE_ENTITLEMENT:
            #print("TYPE_REQUIRE_ENTITLEMENT diff")
            return False
        ...
        #return self.TYPE_SINGLE==other.TYPE_SINGLE and self.TYPE_START==other.TYPE_START and self.TYPE_REQUIRE_ANY==other.TYPE_REQUIRE_ANY and self.TYPE_REQUIRE_ALL==other.TYPE_REQUIRE_ALL and self.TYPE_REQUIRE_ENTITLEMENT==other.TYPE_REQUIRE_ENTITLEMENT and self.type==other.type and self.is_not==other.is_not and self.value==other.value and self.decision==other.decision
"""
    def lst__eq__(self, one, two): # to evaluate the equality of two lists of ReducedVertice # ADD
        if len(one)==len(two):
            for i in range(len(one)):
                if not one[i].__eq__(two[i]):
                    return False
            return True
        return False
    ...
    def _replace_in_list(self, lst, old, new): # doesn't return anything, so we don't keep the changes in mind + pbl of iteration of return ? if we want to change more than 1 vertice. I write after a proposition of correction
        ...
        for i, v in enumerate(tmp_list):
            if isinstance(v.value, list):
                ...
                if v == old:
                    ...
                    return # if here we just change maximum 1 object

    def _replace_in_list_corr(self, lst, old, new):
        global replace_occurred
        tmp_list = list(lst)
        for i, v in enumerate(tmp_list):
            if isinstance(v.value, list):
                v.value = self._replace_in_list_corr(v.value, old, new) # ADD of v.value=
            else:
                if v == old:
                    lst[i] = new
                    replace_occurred = True
        return lst # ADD lst + supress the iterations

    def replace_in_list(self, old, new):
        if isinstance(self.value, list):
            self.value = self._replace_in_list_corr(self.value, old, new) # ADD of the '_corr' at the end + 'self.value='
    ...
    def _replace_sublist_in_list(self, lst, old, new):
        ...
        for i, v in enumerate(lst):
            if isinstance(v.value, list):
                ...
                return # this will stop the loop for
        ...
        if self.is_type_single():
            if self.is_not and not recursive_is_not:
                ...
                if "(require-any" in value:
                    result_str = self.value.str_not() # str_not() is defined for NonTerminalNode and OperationNode
                else:
        ...
        if self.is_type_single():
            if self.is_not:
                ...
                if "(require-any" in value:
                    ...
                    result_str += "(require-not " + str(self.value) + ")"
                    # result_str = "(require-not " + value + ")"    # to improve
        return result_str
...
class ReducedEdge():
    ...
    def __eq__(self, other): # ADD
        if isinstance(self, ReducedEdge) and isinstance(other, ReducedEdge):
            return self.start==other.start and self.end==other.end
        else:
            return False
    
    def lst_eq_(self, one, other): # ADD, test the equality between 2 lists of edges
        if len(one)==len(other):
            for i in range(len(one)):
                if not one[i].__eq__(other[i]):
                    print("elll", i, one[i], other[i])
                    return False
            return True
        print("no")
        return False
...
class ReducedGraph():
    ...
    def __eq__(self, other): # ADD
        if isinstance(self, ReducedGraph) and isinstance(other, ReducedGraph):
            return self.vertices==other.vertices and self.edges==other.edges and self.final_vertices==other.final_vertices and self.reduce_changes_occurred==other.reduce_changes_occurred
        else:
            return False
        ...
        for e in self.edges:
            if e.start == old:
                ...
                if isinstance(e.start.value, list):
                    e.start.replace_in_list(old, new) # replace each element equal at old in e.start.value to new
                    if replace_occurred:
